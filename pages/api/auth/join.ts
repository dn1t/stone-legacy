...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (username.length < 5 || username.length > 16 || !/^[a-zA-z0-9]{4,12}$/.test(username))
    return res.status(422).json({ message: '아이디는 5자 이상, 16자 이하의 영어 대소문자, 숫자로 이루어져야 합니다.', error: true });
  if (password.trim().length < 7 || password.trim().length > 36 || !/^[a-zA-z0-9]{4,12}$/.test(password))
    return res.status(422).json({ message: '비밀번호는 7자 이상, 36자 이하의 영어 대소문자, 숫자로 이루어져야 합니다.', error: true });
